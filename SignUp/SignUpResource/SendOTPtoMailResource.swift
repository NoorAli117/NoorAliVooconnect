...
class SendOTPtoMailResource {
    
    func hittingSendOTPtoMail(sendOTPRequest: SendOTPRequest, complitionHandler : @escaping(Bool, String?) -> Void) {
    ...
    func hittingSendOTPtoPhone(sendOTPRequest: SendOTPRequest, complitionHandler : @escaping(Bool, String?) -> Void) {
        
        var urlRequest = URLRequest(url: URL(string: baseURL + EndPoints.sendOTPtoPhone)!)
        urlRequest.httpMethod = "post"
        urlRequest.addValue("application/json", forHTTPHeaderField: "content-type")
        urlRequest.httpBody = try? JSONEncoder().encode(sendOTPRequest)
