...
class MainBottomFunctionView: UIView {
    ...
    @IBOutlet weak var contentView: ARGearContentView!
...
//        self.shutterView.setRatio(ratio)
    ...
    @IBAction func bottomButtonAction(_ sender: UIButton) {
       ...
       bottumButtonAction(index: sender.tag)
    }
    func bottumButtonAction(index: Int) {
        self.openBottomFunctions()
        
        let buttonType = MainBottomButtonType(rawValue: index)
        closeAll()
        switch buttonType {
        case .Beauty:
